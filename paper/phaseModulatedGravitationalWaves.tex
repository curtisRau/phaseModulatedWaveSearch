...
It is not explicitly obvious how The Gustafson Algorithm, Eq. \ref{eqn:gustafsonAlgorithm}, is able to determin the values $\omega_0, \omega_1, \phi_0, \phi_1, \Gamma$.  In this section we seek to make obvious what Eq. \ref{eqn:gustafsonAlgorithm} can do.  First we start with Eq. \ref{eqn:gustafsonAlgorithm}, and plug in the Fourier Transform of the real of the waveform \ref{eqn:realFourierTransform}.

\begin{align}
h_0 \delta (\omega - \omega_0) 
&= \sum_{n=-\infty}^{\infty} e^{-in(\widetilde{\phi_1}+\pi/2)} J_n(\widetilde{\Gamma}) \hat{f} (\omega + n\widetilde{\omega_1})	\\
