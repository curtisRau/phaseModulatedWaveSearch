...
\title{A Novel Method for Removing Cyclic
Frequency Modulation in Fourier Space}
...
\item researched and wrote sections on error analysis,  wrote the results-discussion and conclusion, wrote parts of the introduction, MATLAB code for noise section
\end{itemize}
...
<<<<<<< HEAD
For clarity and later convenience let's have $n\to -n$.  We will use the other property of the Bessel Functions: \cite{kreh}
=======
...
<<<<<<< HEAD
What would we have to do to get the fourier transform of the complex wave to look exactly like the fourier transform of the real wave?  Unsurprisingly not much because they are such similar functions.  All we would have to do is 
=======
...
We would only have to multiply each term by some phase factor to get the real fourier transform to look like the sum of two complex fourier transforms.  After adding in the phase factors we plug the  fourier transform of the real wave into the Gustafson Algorithm which will yield two delta functions now instead of one (because the fourier transform of the real is the sum of two complex wave fourier transforms).  One delta function will be at $+\omega_0$, this is the one of interest, and the other will be at $-\omega_0$.  (A much more rigorous explanation of this is needed.)  If we carry the search out over only positive values of $\omega_0$ then we can throw out all terms in red of the fourier transform of the real wave because they are inconsequential to the algorithm.  Finally, the phase terms that were added to the fourier transform of the real to make it look like the complex can be incorporated into the Guatafson Algorithm.  The only remaining thing to do is multiply the Gustafson Algorithm by two (explain why).
=======
...
As mentioned previously, quantum noise --- in particular, thermal noise --- has been implicated as a limiting factor in the sensitivity of gravitational wave interferometers and hence a prime factor in the degradation of the signal quality \citep{Saulson}. Following from this, it can be inferred that the distribution of the noise is approximately Gaussian.
...
\begin{figure*}[ht]
	...
	\begin{subfigure}{.5\textwidth}
  		...
  		\caption{\label{fig:denoise_3} Output wrt MSE and SNR}
	\end{subfigure}%
	\begin{subfigure}{.5\textwidth}
  		\centering
  		\includegraphics[width=1.15\linewidth]{denoise_4.jpg}
  		\caption{\label{fig:denoise_4} Behavior of the proposed algorithm under different correlations and SNRs}
	\end{subfigure}
...
<<<<<<< HEAD
if we are willing to sacrifice some precision to truncation we can use a double precision floating point integer which allows values up to $1.797,693,134,862,315,7 \cdot 10^{308}$
=======
...
<<<<<<< HEAD
This is a tridiagonal matrix system that characterizes a one dimensional differential equation.  It can be easily solved using the standard Thomas Algorithm.  To program this in C++ we need all the arrays to be indexed from 0, so 
=======
...
\subsection{Noise Sources}
Since the signal being searched for is relatively stable but of little amplitude, the reliability of data obtained from the detector is related to the degree of noise therein. Among others, quantum, thermal, seismic and Newtonian noise have been implicated as prime drivers of LIGO data adulteration \citep{Saulson}. Since the prime noise contributor is quantum, a summation of all the noise sources are approximately Gaussian. Thus, it suffices to use the properties of Gaussian noise to understand the volatility of the data.
...
Thereafter, the fast Fourier transform of each intrinsic mode function in $\mathbf{L}$ is computed to obtain a matrix $\mathcal{F} \left( \mathbf{L} \right)$. Singular value decomposition of the variance-covariance matrix $\Sigma$ obtained from $\mathcal{F}{ \left( \mathbf{L} \right) }$ is then computed to result in the following:
\begin{equation}
\Sigma = \mathbf{USV}
\end{equation}
...
y = \underset{b}{\operatorname{argmin}} \left( \Bigg| \frac{\sum_{j=1}^{(b)} \lambda_j}{trace \{ \mathbf{S} \}} - \frac{\Delta}{100} \Bigg| \right)
\end{equation}
where $y$ is the index denoting the $y^\textnormal{th}$ eigenvalue from $\lambda_y$ with $\lambda = diag{(\mathbf{S})}$. It is essential to note that the eigenvalues are ordered from the highest to the lowest; the eigenvectors are also ordered by the position of their corresponding eigenvalues.
...
The fast Fourier transforms are then recorrelated to obtain denoised Fourier transforms of intrinsic mode functions in the following manner:
\begin{equation}
\tilde{ \mathbf{L} } = \mathbf{V}\mathbf{R}.
\end{equation}
...
\begin{figure}[H]
	...
	\end{subfigure}
<<<<<<< HEAD
	\caption{\label{fig:gustafsonAlgoResults} Sampling Rate = 300Hz, Carrier = 20Hz, Mod Frequency = 6Hz, Mod Index = 10, Time Series Duration = 10s, SNR = 0.1}
\end{figure*}


\section{Conclusion}
Recap the benefits of working in the frequency domain vs the time domain.
...
=======
	\caption{\label{fig:} Application of the Gustafson algorithm to the noisy frequency modulated signal illustrated in Fig. \ref{fig:results2}}
\end{figure}
